...
{
    const std::string file_path = GetResourcePath(Resource::Type::FONT, id) + FONT_EXTENSION;
...
void Hachiko::FontImporter::Save(UID id, const Resource* resource)
{
    ...
    const std::string lib_font_path = GetResourcePath(Resource::Type::FONT, id) + FONT_EXTENSION;
