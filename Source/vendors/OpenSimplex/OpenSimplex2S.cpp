...
void OpenSimplex2S::initLatticePoints()
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        auto c0 = std::make_shared<LatticePoint3D>(LatticePoint3D(i1, j1, k1, 0));
        auto c1 = std::make_shared<LatticePoint3D>(LatticePoint3D(i1 + i2, j1 + j2, k1 + k2, 1));

        // (1, 0, 0) vs (0, 1, 1) away from octant.
        auto c2 = std::make_shared<LatticePoint3D>(LatticePoint3D(i1 ^ 1, j1, k1, 0));
        auto c3 = std::make_shared<LatticePoint3D>(LatticePoint3D(i1, j1 ^ 1, k1 ^ 1, 0));

        // (1, 0, 0) vs (0, 1, 1) away from octant, on second half-lattice.
        auto c4 = std::make_shared<LatticePoint3D>(LatticePoint3D(i1 + (i2 ^ 1), j1 + j2, k1 + k2, 1));
        auto c5 = std::make_shared<LatticePoint3D>(LatticePoint3D(i1 + i2, j1 + (j2 ^ 1), k1 + (k2 ^ 1), 1));

        // (0, 1, 0) vs (1, 0, 1) away from octant.
        auto c6 = std::make_shared<LatticePoint3D>(LatticePoint3D(i1, j1 ^ 1, k1, 0));
        auto c7 = std::make_shared<LatticePoint3D>(LatticePoint3D(i1 ^ 1, j1, k1 ^ 1, 0));

        // (0, 1, 0) vs (1, 0, 1) away from octant, on second half-lattice.
        auto c8 = std::make_shared<LatticePoint3D>(LatticePoint3D(i1 + i2, j1 + (j2 ^ 1), k1 + k2, 1));
        auto c9 = std::make_shared<LatticePoint3D>(LatticePoint3D(i1 + (i2 ^ 1), j1 + j2, k1 + (k2 ^ 1), 1));

        // (0, 0, 1) vs (1, 1, 0) away from octant.
        auto cA = std::make_shared<LatticePoint3D>(LatticePoint3D(i1, j1, k1 ^ 1, 0));
        auto cB = std::make_shared<LatticePoint3D>(LatticePoint3D(i1 ^ 1, j1 ^ 1, k1, 0));

        // (0, 0, 1) vs (1, 1, 0) away from octant, on second half-lattice.
        auto cC = std::make_shared<LatticePoint3D>(LatticePoint3D(i1 + i2, j1 + j2, k1 + (k2 ^ 1), 1));
        auto cD = std::make_shared<LatticePoint3D>(LatticePoint3D(i1 + (i2 ^ 1), j1 + (j2 ^ 1), k1 + k2, 1));

        // First two points are guaranteed.
        c0->nextOnFailure = c0->nextOnSuccess = c1.get();
        c1->nextOnFailure = c1->nextOnSuccess = c2.get();

        // If c2 is in range, then we know c3 and c4 are not.
        c2->nextOnFailure = c3.get();
        c2->nextOnSuccess = c5.get();
        c3->nextOnFailure = c4.get();
        c3->nextOnSuccess = c4.get();

        // If c4 is in range, then we know c5 is not.
        c4->nextOnFailure = c5.get();
        c4->nextOnSuccess = c6.get();
        c5->nextOnFailure = c5->nextOnSuccess = c6.get();

        // If c6 is in range, then we know c7 and c8 are not.
        c6->nextOnFailure = c7.get();
        c6->nextOnSuccess = c9.get();
        c7->nextOnFailure = c8.get();
        c7->nextOnSuccess = c8.get();

        // If c8 is in range, then we know c9 is not.
        c8->nextOnFailure = c9.get();
        c8->nextOnSuccess = cA.get();
        c9->nextOnFailure = c9->nextOnSuccess = cA.get();

        // If cA is in range, then we know cB and cC are not.
        cA->nextOnFailure = cB.get();
        cA->nextOnSuccess = cD.get();
        cB->nextOnFailure = cC.get();
        cB->nextOnSuccess = cC.get();

        // If cC is in range, then we know cD is not.
        cC->nextOnFailure = cD.get();
